...
class TaskGraph(TaskGraphBase):
    ...
    def create_graph(self):
        ...
        for edge in edges:
            if isinstance(edge[2]['intent'], str):
                edge[2]['intent'] = edge[2]['intent'].lower()
            else:
                print("DEBUG: Intent type:", type(edge[2]['intent']))
                print("DEBUG: Intent content:", edge[2]['intent'])

                #print("DEBUG: Unexpected intent format ->", edge[2]['intent'])
                edge[2]['intent'] = "task_flow_intent"
            #edge[2]['intent'] = edge[2]['intent'].lower()
    ...
    def create_graph(self):
        ...
        for edge in edges:
            intent = edge[2]['intent']
            if isinstance(intent, dict) and 'task_name' in intent:
                edge[2]['intent'] = intent['task_name'].lower()
            elif isinstance(intent, str):
                edge[2]['intent'] = intent.lower()
            else: 
                edge[2]['intent'] = "task_flow_intent"
                #print("STILL HERE IN DEFAULT TASKGRAPH")
