...
class Generator:
    def __init__(self, args, config, model, output_dir, resource_inizializer: Optional[BaseResourceInitializer]  = None):
        ...
        #print("DEBUG: product_kwargs content ->", self.product_kwargs)
        workers_list = []
        for task in self.product_kwargs.get("tasks", []):
            workers_list.extend(task.get("workers", []))
        self.workers = resource_inizializer.init_workers(workers_list)
    ...
    def generate(self):
        ...
        #print("DEBUG: Task Generation Process Started")
        #print("DEBUG: Task Graph Input Data:", self.product_kwargs)
        hitl_result = app.run()
        ...
        #print("DEBUG: Checking hitl_result before iteration:", hitl_result)  # Debug statement
